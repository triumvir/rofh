  ...
  const firstDayOfMonth = React.useMemo(() => {
    ...
    for (let i = 0; i < days.length; i += 1) {
      ...
      for (let j = 0; j < dates.length; j += 1) {
        ...
        const first = hideOutsideDates ? isSameDate.isSameDate(date, dayjs__default(month).startOf("month").toDate()) : j === 0 && i === 0;
  ...
  const rows = days.map((row, rowIndex) => {
    const cells = row.map((date, cellIndex) => {
      ...
      return /* @__PURE__ */ React__default.createElement("td", {
        ...
        firstInMonth: isSameDate.isSameDate(date, firstDayOfMonth),
